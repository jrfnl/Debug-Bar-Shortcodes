 ...
 */
if ( ! class_exists( 'Debug_Bar_Shortcodes_Renderer' ) && class_exists( 'Debug_Bar_Shortcodes' ) ) {
	// Extending Debug Bar
	class Debug_Bar_Shortcodes_Renderer {
...
//			self::$name = Debug_Bar_Shortcodes::DBS_NAME;
...
//			add_filter( 'db_shortcodes_info', array( $this, 'reflection_retrieve_shortcode_info' ), 12, 2 ); // Last option, will only run if info is bare
			...
			if ( is_array( $shortcodes ) && $shortcodes !== array() ) {
				...
				foreach ( $shortcodes as $shortcode => $callback ) {
					...
					$output .= '
						<tr' . $class . '>
							...
							<td class="column-title">
								...
								' . $this->get_action_links( $shortcode, $has_details, $info ) . '
							</td>
					...
					if ( $is_singular === true ) {
						...
						$output .= '
							<td>' . $this->get_image( array( 'true' => __( 'Shortcode is used', self::$name ), 'false' => __( 'Shortcode not used', self::$name ) ), $in_use, true ) . '</td>
							<td>' . ( $in_use === true ? $this->get_shortcode_usage( $shortcode ) : '&nbsp;' ) . '</code></td>';
						unset( $in_use );
					}
					...
					if ( $has_details === true ) {
						...
						<tr' . $class . '>
							...
							<td colspan="' . ( $is_singular === true ? 5 : 3 ) . '">
								' . $this->get_details_table( $shortcode, $info ) . '
							</td>
						</tr>
						';
					}
		 ...
		 * @return	string
		 */
		public function get_action_links( $shortcode, $has_details, $info ) {
			$links = array();
			...
			if ( $has_details === true && $info['info_url'] !== '' ) {
				$links[] = '<a href="' . esc_url( $info['info_url'] ) . '" target="_blank" title="' . __( 'View extended info about the shortcode on the web', self::$name ) . '" class="' . esc_attr( self::$name . '-external-link' ) . '" >' . esc_html__( 'View online', self::$name ) . '</a>';
			}
			
			return '<div class="row-actions">' . implode( ' | ', $links ) . '<span class="spinner"></span></div>';
		}
		 ...
		 * @return	string
		 */
		public function get_details_table( $shortcode, $info ) {
			$rows = array();
...
//pr_var( $info['self_closing'], $shortcode . ': self_closing' );
		 ...
		 * @return	string
		 */
		public function get_image( $alt = array(), $bool = null, $show_false = false, $show_null = false ) {
			static $images;
			...
			$alt_tag = ( ( is_string( $alt ) && $alt !== '' ) ? ' alt="' . esc_attr( $alt_tag ) . '"' : '' );


			$return = '';
			if ( ( $bool === true || ( $bool === false && $show_false === true ) ) || ( $bool === null && $show_null === true ) ) {
				$return = '<img src="' . esc_url( $img ) . '" width="16" height="16"' . $alt_tag . '/>';
			}
			return $return;
		}
		...
		public function has_shortcode( $shortcode ) {
			...
			if ( ! isset( $matches ) || ! isset( $matches[$post_id] ) ) {
		 ...
		 * @return	string
		 */
		public function get_shortcode_usage( $shortcode, $content = null ) {
...
//pr_var( $shortcode, '<h1>$shortcode</h1>' );
...
//pr_var( $info, 'info before validation' );
			$info = $this->validate_shortcode_info( $info );
//pr_var( $info, 'info after validation' );
			return $info;
		}
...
//pr_var( $additional, 'info after reflection' );

//pr_var( self::array_merge_recursive_distinct( $info, $additional ), 'info after merge' );
				return self::array_merge_recursive_distinct( $info, $additional );
			}
		}
...
//			$info = $this->info_defaults;
			...
			$info['description'] = $this->parse_doc_comment( $reflection->getDocComment() );

//pr_var( $reflection->getParameters(), 'parameters' );
//pr_var( $reflection->getNumberOfRequiredParameters(), 'nr of req parameters' );
			$info['self_closing'] = true;
			if ( $reflection->getNumberOfRequiredParameters() > 1 ) {
				$info['self_closing'] = false;
			}

			$info['info_url'] = $this->get_plugin_info_url_from_file( $reflection->getFileName(), $shortcode );
//pr_var( $info['info_url'], 'url as retrieved' );


//pr_var( $info, 'info reflection result' );
			return $info;
		}


		public function parse_doc_comment( $comment ) {
		 ...
		 * @return  string  url
		 */
		public function get_plugin_info_url_from_file( $path_to_file, $shortcode ) {
			...
			$wpmu_plugins_path = str_replace( array( '/', '\\' ), DIRECTORY_SEPARATOR, WPMU_PLUGIN_DIR ) . DIRECTORY_SEPARATOR;

//pr_var( $path_to_file, '$path_to_file' );
//pr_var( $wp_abs_path, '$wp_abs_path' );
//pr_var( $wp_includes_path, '$wp_includes_path' );
//pr_var( $wp_plugins_path, '$wp_plugins_path' );
//pr_var( $wpmu_plugins_path, '$wpmu_plugins_path' );
			...
			$is_mu_plugin    = strpos( $path_to_file, $wpmu_plugins_path );
//pr_var( $is_plugin, '$is_plugin' );
//pr_var( $is_mu_plugin, '$is_mu_plugin' );
			$plugin_data     = array();
			$plugin_basename = '';

			/* Is this a plugin in the normal plugin directory ? */
			if ( is_admin() && $is_plugin !== false ) {
				// Plugin in the plugins directory
				$use_path = substr( $path_to_file, ( $is_plugin + strlen( $wp_plugins_path ) ) );
				if ( strpos( $use_path, DIRECTORY_SEPARATOR ) !== false ) {
					// Subdirectory plugin
					$use_path = substr( $use_path, 0, strpos( $use_path, DIRECTORY_SEPARATOR ) );
					$use_path = DIRECTORY_SEPARATOR . $use_path . DIRECTORY_SEPARATOR;
//pr_var( $use_path, 'use path within is_plugin' );
					$plugins  = get_plugins( $use_path );
//pr_var( $plugins, 'plugins found' );
					...
					$plugin_basename = $use_path;
					$plugin_data     = get_plugin_data( $path_to_file, false, false );
				}
			}
			/* Is this a plugin in the mu plugin directory ? */
			else if ( is_admin() && $is_mu_plugin !== false ) {
				$use_path = substr( $path_to_file, ( $is_mu_plugin + strlen( $wpmu_plugins_path ) ) );
				if ( strpos( $use_path, DIRECTORY_SEPARATOR ) !== false ) {
					// Subdirectory file, presume the mu-dir plugin bootstrap file is called directoryname.php
					$use_path = substr( $use_path, 0, strpos( $use_path, DIRECTORY_SEPARATOR ) );
					$use_path = $use_path . '.php';
				}
				$plugin_basename = $use_path;
				$plugin_data     = get_plugin_data( $wpmu_plugins_path . $use_path, false, false );
			}
//pr_var( $plugin_basename, 'plugin basename' );
//pr_var( $plugin_data, 'plugin_data' );
		...
		public function wp_repo_exists( $plugin_basename ) {
			...
			/* Efficiency - only initialize curl once and keep the resource for re-use */
			if ( false === is_resource( $curl ) ) {
				$curl = curl_init();
...
//pr_var( $plugin_basename, 'basename within curl' );
			if( $plugin_basename !== '' ) {
				//$esc_basename = curl_escape( $curl, substr( $plugin_basename, 0, strpos( $plugin_basename, '/' ) ) ); // PHP5.5+
				$plugin_uri   = 'http://wordpress.org/plugins/' . urlencode( $plugin_basename );
//pr_var( $plugin_uri, 'plugin uri within curl' );
		...
		function ajax_retrieve_details( $shortcode ) {
			...
			$response = array(
				'id'        => 1,
				'data'      => $this->get_details_table( $shortcode, $info ),
		...
		function ajax_find_shortcode_uses( $shortcode ) {
			...
			foreach ( $posts as $i => $post ) {
				...
				if ( $can_edit_post && 'trash' != $post->post_status ) {
					/* TRANSLATORS: no need to translate, WP standard translation will be used */
					$actions['edit'] = '<a href="' . $edit_link . '" title="' . esc_attr( __( 'Edit this item' ) ) . '">' . __( 'Edit' ) . '</a>';
				}
				...
				$output .= '
									...
									<td>' . $this->get_shortcode_usage( $shortcode, $post->post_content ) . '</td>
								</tr>';
			}
			$output .= '
							</tbody>
